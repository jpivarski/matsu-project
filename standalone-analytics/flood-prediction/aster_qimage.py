...
def gridloop(key, picture, decimation, maxregions, granularity, timestamp, outfile = None):
  ...
  png2(origarray[:,:,0], origarray[:,:,0], origarray[:,:,0], fileName = key + '.png') #, lines=foci)
  ...
  for ipass in range(maxregions):
    ...
    avg10x10 = scipy.signal.convolve2d(pixelarray[:,:,0], numpy.ones((10,10))/100, 'valid')
    avg40x40 = scipy.signal.convolve2d(pixelarray[:,:,0], numpy.ones((40,40))/1600, 'valid')
  ...
  if not isinstance(inputStream, str):
    ...
    while True:
      ...
      try:
        ...
        continue
    gridloop('test', picture, 1., 2, 5, '00:00:00')
  else:    
    picture = asterPicture(os.path.join('.', inputStream))
    ...
    for suffix in ['q0','q1','q2','q3']:
      if len(inputStream.split(suffix))==2:
        break
    key = inputStream.split(suffix)[0]
    timestamp = None

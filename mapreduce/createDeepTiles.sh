...
for SMALLCHUNK in '/user/hadoop/sequence-files/eo1/ali_l1g/2012/2[3-5]*/*.sequence' \
    ...
    (echo /opt/hadoop/bin/hadoop jar /opt/hadoop/contrib/streaming/hadoop-streaming-1.0.3.jar -D org.occ.matsu.restrictBands=$RESTRICT_BANDS -D "org.occ.matsu.restrictBandsTo='`cat /tmp/bands.json`'" -D mapred.reduce.tasks=0 -D mapred.min.split.size=107374182400 -libjars /opt/matsuSequenceFileInterface.jar -input $SMALLCHUNK -mapper "\"mapper-L1G-tiles-Accumulo.py /tmp/bands.json\"" -inputformat org.occ.matsu.UnsplitableSequenceFileInputFormat -file mapper-L1G-tiles-Accumulo.py -file utilities.py -file ../jobconfig.ini -cmdenv PYTHONPATH=/opt/lib/python -cmdenv LD_LIBRARY_PATH=/opt/avrocpp-1.7.1/lib -output $OUTPUTDIR-$x > $LOGFILE-$x.log &)
